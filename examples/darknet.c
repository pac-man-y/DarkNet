...
//darknet的入口函数，根据给的参数来调用不同的框架，这个代码框架还是非常大的
...
/*
比对字符串来选择，这个才是主要的，然后需要用到不同的功能的话去看不同的子文件的代码
如果需要在这个框架上做点什么的话，主要改的是这里选择的不同的代码文件，src文件里面存的是底层
的框架文件，比如cuda代码，网络组成的代码等最好是不要动了。
*/
